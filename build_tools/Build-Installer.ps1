...
Remove-Item -Force -ErrorAction SilentlyContinue (Join-Path $InstallerOutput 'MinutasASH_Setup_2.3.5_Online.exe')
Remove-Item -Force -ErrorAction SilentlyContinue (Join-Path $InstallerOutput 'MinutasASH_Setup_2.3.5_Online_SHA256.txt')
& $Iscc (Join-Path $Root 'installer\MinutasASH.iss')
if ($LASTEXITCODE -ne 0) { throw 'Inno Setup no pudo compilar el instalador.' }

$Setup = Join-Path $Root 'dist_installer\MinutasASH_Setup_2.3.5_Online.exe'
...
$HashFile = Join-Path $Root 'dist_installer\MinutasASH_Setup_2.3.5_Online_SHA256.txt'

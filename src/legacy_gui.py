...
class MinutasApp(tk.Tk):
    ...
    def _metadata_from_form(self) -> MeetingMetadata:
        payload = {key: variable.get().strip() or None for key, variable in self.meta_vars.items()}
    ...
    def start_analysis(self) -> None:
        ...
        if descriptor.is_remote and bool(self.config_data.get("confirm_remote_processing", True)):
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    self._apply_processing_telemetry(payload)
                    ...
                    self._apply_update_downloaded(payload)
    ...
    def _apply_processing_telemetry(self, payload: object) -> None:
        ...
        if event_type == "processing_plan":
        ...
        elif event_type == "resource_recheck":
            profile = payload.get("effective_profile") or {}
            if isinstance(profile, dict):
                state["profile_name"] = profile.get("display_name")
            snapshot = payload.get("resource_snapshot") or {}
            if isinstance(snapshot, dict):
                state["memory_percent"] = snapshot.get("memory_percent")
                state["available_memory_bytes"] = snapshot.get("available_memory_bytes")
            previous_name = payload.get("previous_profile_name")
            current_name = state.get("profile_name") or "Rápido"
            if previous_name and previous_name != current_name:
                self._log(
                    f"Memoria comprobada con el modelo cargado: perfil ajustado de {previous_name} a {current_name}."
                )
            else:
                self._log(f"Memoria comprobada con el modelo cargado: perfil {current_name}.")
    ...
    def _apply_provider_status(self, payload: object) -> None:
    ...
    def _apply_ollama_status(self, payload: object) -> None:
        self._apply_provider_status(payload)
    ...
    def _apply_update_downloaded(self, payload: object) -> None:
        info, path = payload
        self._set_busy(False)
        self.progress_var.set(100)
    ...
    def _log(self, message: str) -> None:
        ...
        try:
            maximum = int(self.config_data.get("activity_log_max_lines", 600))
            lines = int(self.log_text.index("end-1c").split(".")[0])
            if lines > maximum:
                self.log_text.delete("1.0", f"{lines - maximum + 1}.0")

...
class AdministrationCenter(tk.Toplevel):
    ...
    def _refresh_catalog(self) -> None:
        ...
        for row in self._catalog_rows(catalog):
            key = row.get("code") if catalog == "projects" else row.get("id")
            values = []
            ...
            self.catalog_tree.insert("", "end", iid=str(key), values=values)
    ...
    def _import_catalog(self) -> None:
        ...
        try:
            summary = import_catalog(
                ...
                path,
                duplicate_policy=str(self.config.get("catalog_import_duplicate_policy", "upsert")),
            )
            detail = (
    ...
    def _create_backup(self) -> None:
        try:
            path = create_backup(self.database, app_version=str(self.config.get("app_version") or ""))

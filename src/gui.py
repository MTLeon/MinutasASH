"""Interfaz guiada de Minutas ASH 2.3.3.
...
from src.audio_transcription import SUPPORTED_MEDIA_SUFFIXES, transcribe_media
...
class GuidedMinutasApp(LegacyMinutasApp):
    ...
    def new_minute(self) -> None:
        ...
        if any(self.meta_vars[key].get().strip() for key in meaningful_keys) or self.vtt_var.get().strip() or self.items:  # noqa: SIM102
    ...
    def _metadata_from_form(self) -> MeetingMetadata:
        ...
        metadata.source_type = cast(SourceType, source_type) if source_type in SOURCE_TYPE_LABELS else "vtt"
        metadata.source_quality = cast(SourceQuality, source_quality) if source_quality in SOURCE_QUALITY_LABELS else "media"
    ...
    def _on_vtt_drop(self, event) -> str:
        ...
        for raw in paths:
            ...
            if candidate.suffix.casefold() in ({".vtt", ".txt", ".docx"} | SUPPORTED_MEDIA_SUFFIXES) and self._accept_input_path(candidate):
                return "break"
        messagebox.showwarning("Fuente de reunión", "Arrastre VTT, TXT, DOCX, audio o video.", parent=self)
        return "break"
    ...
    def _build_review_tab(self) -> None:
        ...
        self.review_context_menu.add_command(label="Descartar selección", command=lambda: self._set_selected_review_status("descartado"))
        ...
        ttk.Button(review_buttons, text="Descartar selección", command=lambda: self._set_selected_review_status("descartado")).grid(row=1, column=0, sticky="ew", padx=(0, 4))
        ...
        ttk.Button(self.advanced_review_controls, text="Eliminar", command=self.delete_item).pack(side="left", padx=6)
    ...
    def _refresh_items_tree(self, select_index: int | None = None) -> None:
        ...
        only_pending = bool(self.review_filter_var.get()) if hasattr(self, "review_filter_var") else False
        ...
        if path.is_file():
            try:
                ...
                if target is not None and segments:
                    nearest = min(
                        range(len(segments)),
                        key=lambda position: abs((self._seconds_from_timestamp(segments[position].start) or 0) - target),
                    )
